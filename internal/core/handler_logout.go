...
func (t *TLDR) Logout(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		t.errorResponse(w, r.Context(), http.StatusBadRequest, "Invalid/missing cookie")
		return
	}
	...
	if err = t.Queries.RevokeRefreshToken(r.Context(), cookie.Value); err != nil {
		...
		t.errorResponse(w, r.Context(), http.StatusInternalServerError, "Failed to revoke refresh token")
		return
	}
