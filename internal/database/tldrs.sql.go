...
const deleteAllTLDRs = `-- name: DeleteAllTLDRs :execresult
DELETE FROM tldrs
`

func (q *Queries) DeleteAllTLDRs(ctx context.Context) (sql.Result, error) {
	return q.db.ExecContext(ctx, deleteAllTLDRs)
}
...
type DeleteTLDRsByIdAndUserParams struct {
	UserID uuid.UUID   `json:"user_id"`
...
func (q *Queries) GetTLDRsByUser(ctx context.Context, arg GetTLDRsByUserParams) ([]GetTLDRsByUserRow, error) {
	rows, err := q.db.QueryContext(ctx, getTLDRsByUser,
		arg.UserID,
		arg.CreatedAt,
		arg.CreatedAt_2,
		arg.ID,
